...
def loop():
        commandSeq = 'KG1KH2KG1KH2KG1KH2KG1KH2KG1KH2LG1LH2LG1LH2LG1LH2LG1LH2LG1LH2LG1LH2'
        idx = 0
        command = commandSeq[idx];

        while True:
          #command=ser.read()
          #ser.write(command +'\n')#回显
#          command = raw_input("Input command:")
          idx = (idx + 1)%len(commandSeq)
          command = commandSeq[idx]

          print "You inputted:%s"%command
          if command == 'A':
            print "Got A, moving forward"
            t_up(50,0)    #前进
          elif command == 'B':
            t_down(50,0)  #后退
          elif command == 'D':
